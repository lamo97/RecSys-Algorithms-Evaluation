...
def metric_to_csv(metric, field, dataset):
...
def retrieve_delta(path, filename, metric):
    dataframe = pd.read_csv(path + filename)
    return filename_info(filename), dataframe[metric][1]

def delta_to_csv(field,dataset):
                ...
                for cutoff in cutoffs:
                    ...
                    for filename in os.listdir(current_dir):
                        ...
                        if(check_filename(filename, field, candidate, cutoff) == True):
                            ...
                            # inserisce quanto appena ottenuto in un dizionario
                            data['ID'].append(id)
                            if ((f'{metric} | {candidate}') in data) == True:
                                data[f'{metric} | {candidate}'].append(round(val,5))
                            else:
                                data[f'{metric} | {candidate}'] = []
                                data[f'{metric} | {candidate}'].append(round(val,5))
    ...
    metric_df = metric_df.groupby('ID').agg({
        'ID': 'first',
        'DeltaGap - Top 5 | Blockbuster | Test Ratings': sum,
        'DeltaGap - Top 5 | Niche | Test Ratings': sum,
        'DeltaGap - Top 5 | Diverse | Test Ratings': sum,
        'DeltaGap - Top 10 | Blockbuster | Test Ratings': sum,
        'DeltaGap - Top 10 | Niche | Test Ratings': sum,
        'DeltaGap - Top 10 | Diverse | Test Ratings': sum,
        'DeltaGap - Top 10 | Blockbuster | All Items': sum,
        'DeltaGap - Top 10 | Niche | All Items': sum,
        'DeltaGap - Top 10 | Diverse | All Items': sum,  
        'DeltaGap - Top 20 | Blockbuster | All Items': sum,
        'DeltaGap - Top 20 | Niche | All Items': sum,
        'DeltaGap - Top 20 | Diverse | All Items': sum  
    })
    

    # salva il dataframe in un csv
    metric_df.to_csv(result_path + f'[{dataset}] DeltaGap - {field}.csv', index=False)


metric_to_csv('Recall','genres', '1M')
metric_to_csv('F1','genres', '1M')
metric_to_csv('NDCG','genres', '1M')
metric_to_csv('Gini','genres', '1M')
metric_to_csv('CatalogCoverage','genres', '1M')
delta_to_csv('genres','1M')