def predict(recommender, current_run):
    ...
    for train_set, test_set in zip(train_list, test_list):
        ...
        rank_to_append = cbrs.fit_rank(test_set, methodology=rs.AllItemsMethodology())
        result_list.append(rank_to_append)

    for result in result_list:
        print(result)

    evaluation(result_list, test_list, current_run['algorithm'], current_run['methodologyID'])
...
def evaluation(result_list, test_list, current_run):
    ...
    for cutoff in cutoff_list:
        em = eva.EvalModel(
            ...
            metric_list=[
                eva.PrecisionAtK(cutoff),
                eva.RecallAtK(cutoff),
                eva.FMeasureAtK(cutoff),
                eva.NDCGAtK(cutoff),
                eva.MRRAtK(cutoff),
                eva.GiniIndex(),
                eva.CatalogCoverage(),
        ...
        sys_result.to_csv(
            results_path + current_run['fields'] + '/' + 'SYS - ' + sys.argv[1] 
            + ' - ' + current_run['algorithm'] + ' (' + current_run['methodologyID@'] + cutoff + ').csv')
