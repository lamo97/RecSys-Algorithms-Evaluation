...
while(True):
    ...
    if(alg_choice == 1):
        centroid_vector = rs.CentroidVector({fields[0]: [representations[choice-1]],
                                             fields[1]: [representations[choice-1]]}, similarity = rs.CosineSimilarity())
        predict(centroid_vector, current_setup)

    elif(alg_choice == 2):
        logistic_regression = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                                        fields[1]: [representations[choice-1]]}, rs.SkLogisticRegression())
        predict(logistic_regression, current_setup)

    elif(alg_choice == 3):
        random_forest = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                                  fields[1]: [representations[choice-1]]}, rs.SkRandomForest(n_estimators = 145))
        predict(random_forest, current_setup)

    elif(alg_choice == 4):
        svc = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                        fields[1]: [representations[choice-1]]}, rs.SkSVC())
        predict(svc, current_setup)

    elif(alg_choice == 5):
        knn = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                        fields[1]: [representations[choice-1]]}, rs.SkKNN())
        predict(knn, current_setup)

    elif(alg_choice == 6):
        decision_tree = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                                  fields[1]: [representations[choice-1]]}, rs.SkDecisionTree())
        predict(decision_tree, current_setup)

    elif(alg_choice == 7):
        gaussian_process = rs.ClassifierRecommender({fields[0]: [representations[choice-1]],
                                                     fields[1]: [representations[choice-1]]}, rs.SkGaussianProcess())
        predict(gaussian_process, current_setup)

    elif(alg_choice == 8):
        gradient_descent = rs.LinearPredictor({fields[0]: [representations[choice-1]],
                                               fields[1]: [representations[choice-1]]}, rs.SkSGDRegressor())
        predict(gradient_descent, current_setup)

    elif(alg_choice == 9):
        ridge_reg = rs.LinearPredictor({fields[0]: [representations[choice-1]],
                                        fields[1]: [representations[choice-1]]}, rs.SkRidge())
